// netlify/functions/finalize-pending-newsletter.js
//
// Step 2 of the submit → finalize flow. Takes the (possibly edited) content
// for a pending newsletter and actually creates the ActiveCampaign draft —
// same merge/creation logic as create-campaign.js — then marks the pending
// record as "completed" so it drops out of the active queue but stays
// visible in the Finalize tab's history list.
...
exports.handler = async (event) => {
  ...
  let id, campaignName, introText, listId, templateId, stories;
  try {
    ({ id, campaignName, introText, listId, templateId, stories } = JSON.parse(event.body));
  } catch {
    return { statusCode: 400, body: JSON.stringify({ error: 'Invalid JSON body' }) };
  }

  if (!id || !campaignName || !Array.isArray(stories) || stories.length === 0) {
    return {
      statusCode: 400,
      body: JSON.stringify({ error: 'id, campaignName, and a non-empty stories array are required' }),
    };
  }
    ...
    try {
      ...
      if (existing) {
        await store.setJSON(id, {
          ...
          resultCampaignId: campaignRes.campaign_create?.id || campaignRes.id || null,
          resultMessageId: messageId,
        });
      }
    } catch {
      // Don't fail the whole request just because the status update failed —
      // the AC draft was already created successfully at this point.
    }
